...
// after every wallpaper change. Every colour comes from the wallpaper, lifted
// only as far as legibility needs; alert and warn are the only fixed
// colours, because colour in this design only ever means "wrong".
import QtQuick
import Quickshell
...
Singleton {
    ...
    // ---- derived -----------------------------------------------------------
    // One name per colour that actually renders. There used to be 48 names
    // for about 25 colours, and some lied: "neonMagenta" was the second
    // accent, a blue. There is no "ok" -- tokyo-palette writes the accent
    // there, because colour only reports what is wrong.
    //
    // Cards are the panel material: a palette layer, translucent over blur.
    readonly property color card:        Qt.rgba(layer1.r, layer1.g, layer1.b, 0.72)
    readonly property color cardDark:    Qt.rgba(bg.r, bg.g, bg.b, 0.76)     // no scrim behind it
    readonly property color cardHover:   Qt.rgba(layer2.r, layer2.g, layer2.b, 0.68)
    readonly property color cardActive:  Qt.rgba(accentDim.r, accentDim.g, accentDim.b, 0.50)
    readonly property color cardPressed: Qt.rgba(accentDim.r, accentDim.g, accentDim.b, 0.70)
    readonly property color socket:      Qt.rgba(layer2.r, layer2.g, layer2.b, 0.55) // unlit meter cell
    // Edges. `line` is the palette's hairline; these are drawn over cards.
    readonly property color edge:        Qt.rgba(1.0, 1.0, 1.0, 0.08)
    readonly property color accentEdge:  Qt.rgba(accent.r, accent.g, accent.b, 0.45)
    readonly property color accentWash:  Qt.rgba(accent.r, accent.g, accent.b, 0.12)
    readonly property color shadow:      Qt.rgba(0.0, 0.0, 0.0, 0.70)
    // 0.75 keeps faint text at ~3.3:1 on a card; 0.5 was 1.75:1, i.e. unread.
    readonly property color faint:       Qt.rgba(dim.r, dim.g, dim.b, 0.75)
    // Tuned against the accent (~3.4:1 on a card). Every JP label draws in
    // the accent; one drawn in `line` would land at 1.5:1.
    ...
    // Five steps, and every size on the desktop is one of them. 10px is the
    // floor for anything a person reads. Literals fold to the nearest step:
    // 11-13 -> szBody, 14-18 -> szValue. The launcher's 44px clock is the
    // one exception left.
    readonly property int szMicro:   10   // labels, units, chips
    readonly property int szBody:    12   // running text, secondary rows
    readonly property int szValue:   15   // the number you read; panel titles
    readonly property int szLead:    20   // section head
    readonly property int szDisplay: 154  // the lock/idle clock
    ...
    // Deck panel widths. Two, plus fullBleed: a switch between views of the
    // same width is a crossfade, not the chassis sliding and resizing. There
    // were fourteen widths for seventeen views. Heights still follow content.
    readonly property int panelS: 860
    readonly property int panelM: 1080
    ...
    // Literals fold by value: <=0.8 tight, 1-2 label, >=2.2 wide.
    readonly property real trkDisplay: -4.0 // szDisplay only
    readonly property real trkTight:    0.5 // mono values
    readonly property real trkLabel:    1.6 // labels
    readonly property real trkWide:     3.2 // headings

    // ---- motion ------------------------------------------------------------
    // Motion starts from the current value and responds on pointer-down.
    // Transitions fold by length: <=150 easeFast, 160-240 ease, 250-450 morph.
    // Loop periods (blinks, spinners) are not transitions and stay literal.
    //
    // Bounce belongs to the island alone (frame/shell.qml's OutBack): it is
    // the one object that behaves physically. Everything else eases out with
    // OutCubic -- a 1000px panel overshooting on every view switch is wobble.
    readonly property real ghost:   0.35  // resting opacity of a stale value
    readonly property int  tapSnapMs: 45  // pointer-down tactile snap
    readonly property int  snapMs:  75    // mechanical tactile snap
    readonly property int  easeFastMs: 120 // hover, focus, small colour changes
    readonly property int  easeMs:  180   // generic state transition
    readonly property int  morphMs: 320   // geometry: panels moving and resizing
